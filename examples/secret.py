...
def create_overwrites(ctx, *objects):
    ...
    A `qq.PermissionOverwrite` allows you to determine the permissions
    of an object, whether it be a `qq.Role` or a `qq.Member`.
    ...
    # for each `qq.Role` or `qq.Member`.
    overwrites = {
        obj: qq.PermissionOverwrite(view_channel=True)
        for obj in objects
    }

    # prevents the default role (@everyone) from viewing the channel
    # if it isn't already allowed to view the channel.
    overwrites.setdefault(ctx.guild.default_role, qq.PermissionOverwrite(view_channel=False))

    # makes sure the client is always allowed to view the channel.
    overwrites[ctx.guild.me] = qq.PermissionOverwrite(view_channel=True)
...
@secret.command()
@commands.guild_only()
async def text(ctx: commands.Context, name: str, *objects: typing.Union[qq.Role, qq.Member]):
...
@secret.command()
@commands.guild_only()
async def voice(ctx: commands.Context, name: str, *objects: typing.Union[qq.Role, qq.Member]):
...
@secret.command()
@commands.guild_only()
async def emoji(ctx: commands.Context, emoji: qq.PartialEmoji, *roles: qq.Role):
